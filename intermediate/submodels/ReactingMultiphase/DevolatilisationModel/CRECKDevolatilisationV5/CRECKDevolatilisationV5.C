...
Foam::CRECKDevolatilisationV5<CloudType>::CRECKDevolatilisationV5
(
...
template<class CloudType>
void Foam::CRECKDevolatilisationV5<CloudType>::calculate
(
  const scalar dt,
  const scalar age,
  const scalar mass0,
...
) const
{
  ...
  for (label i = 0; i < reactionNum; i++)
  {
    ...
    it != gasStoiMolarmassDatas_[i].end(); it++)
    {
        ...
        moleReact*it->second();
        // save the gas phase production of this time step

    }
    // Info << "dMassDV is " << nl << dMassDV << nl;
...
template<class CloudType>
void Foam::CRECKDevolatilisationV5<CloudType>::readReactionDict
(
  const fvMesh& mesh
)
{
  ...
  forAll(reactionDict_,i)
  {
    ...
    forAll(gasMolarMass_, i)
    {
      //W() returns the molar mass,name() returns the species name
