...
Foam::CRECKDevolatilisationV4<CloudType>::CRECKDevolatilisationV4
(
...
template<class CloudType>
void Foam::CRECKDevolatilisationV4<CloudType>::calculate
(
  const scalar dt,
  const scalar age,
  const scalar mass0,
...
) const
{
  ...
  for (label i = 0; i < reactionNum; i++)
  {
    ...
    it != gasStoiMolarmassDatas_[i].end(); it++)
    {
        ...
        // if (gasMolarMass_[productId].name()=="CO2")
        // {
        //   mGas1_ += moleReact*it->second();
        // }
...
template<class CloudType>
void Foam::CRECKDevolatilisationV4<CloudType>::readReactionDict
(
  const fvMesh& mesh
)
{
